...
const _imprimir = async (datos, tipo) => {
    try {
        ...
        const { sucursal, porCategoria, formasPago, adicionales, ventas, totalEfectivo, aperturaCaja } = datos;
        ...
        try {
            ...
            const titulo = tipo === 'RESUMEN' ? 'RESUMEN DE VENTAS' : 'REPORTE GENERAL';
            ...
            if (sucursal?.DIRECCION) {
                await BluetoothEscposPrinter.printText(`Dir: ${sucursal.DIRECCION}\n`, {});
            }
            if (sucursal?.TELEFONO) {
                await BluetoothEscposPrinter.printText(`Tel: ${sucursal.TELEFONO}\n`, {});
            }
            await BluetoothEscposPrinter.printText(`Fecha: ${fechaHoy}\n`, {});
            await BluetoothEscposPrinter.printText(`Ventas: ${adicionales?.NUM_VENTAS ?? 0}\n`, {});
            if (aperturaCaja) {
