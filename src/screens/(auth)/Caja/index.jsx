...
const Caja = () => {
    ...
    const getData = async () => {
        try {
            ...
            console.log("historial", JSON.stringify(res, null, 2));
            setHistorial(res || []);
        } catch (error) {
    ...
    const abrirCaja = async () => {
        ...
        try {
            await Database.insertarApertura({ idSucursal: qrData, nombreDispositivo, monto: montoInicial });
    ...
    const coloresCaja = cajaAbierta ? [gb.green600, gb.green500] : ['#C53030', '#E53E3E'];
    ...
    return (
        <SafeAreaView edges={['bottom']} style={{ flex: 1, backgroundColor: "black" }}>
            ...
            <View style={s.cajaWrapper}>
                <LinearGradient style={s.caja} colors={coloresCaja} start={{ x: 0, y: 0 }} end={{ x: 1, y: 0 }}>
                    <View style={s.cajaTitle}>
                        <Ionicons
                            ...
                            color={gb.gray50}
                        />
                        <Text style={s.cajaText}>{cajaAbierta ? "CAJA ABIERTA" : "CAJA CERRADA"}</Text>
                    </View>
                        ...
                        <View style={s.containerButtons}>
                            {cajaAbierta ? (
                                <Button style={s.button} onPress={() => cerrarCaja()}>
                                    <Ionicons name="lock-closed" size={normalize(12)} color={gb.gray50} />
                                    <Text style={s.text}>Cerrar Caja</Text>
                                </Button>
                            ) : (
                                <Button style={s.button} onPress={() => setModalEditar(true)}>
                                    <Ionicons name="lock-open" size={normalize(12)} color={gb.gray50} />
            ...
            {/* Historial */}
            {historial.length > 0 && (
                <ScrollView style={{ flex: 1, backgroundColor: gb.gray50 }} contentContainerStyle={s.scroll}>
                    <View style={s.historialContainer}>
                        <Text style={s.historialTitle}>Historial</Text>
                        {historial.map((item) => (
                            <View key={item.ID} style={s.historialItem}>
                                <View style={[s.historialIndicador, { backgroundColor: item.ESTATUS === 1 ? gb.green500 : '#E53E3E' }]} />
                                <View style={{ flex: 1 }}>
                                    <Text style={s.historialFecha}>{item.FECHA}</Text>
                                    <Text style={s.historialDevice}>{item.NOMBRE_DISPOCITIVO}</Text>
                                </View>
                                <View style={{ alignItems: 'flex-end' }}>
                                    <Text style={s.historialMonto}>${item.MONTO}</Text>
                                    <Text style={[s.historialEstatusText, { color: item.ESTATUS === 1 ? gb.green500 : '#E53E3E' }]}>
            ...
            >
                <View style={s.contenidoModal}>
                    <Text style={s.modalDescripcion}>Ingrese el monto inicial con el que abre la caja</Text>
                    <Input
                    ...
                    />
                    <Text style={{ color: gb.gray400, fontWeight: "bold", marginTop: normalize(10) }}>Montos rápidos</Text>
                    <View style={s.montosRapidosContainer}>
                        {['100', '200', '500', '1000'].map((monto) => (
                            <Pressable key={monto} style={s.montoRapido} onPress={() => setMontoInicial(monto)}>
                                <Text style={{ color: gb.purple550 }}>${monto}</Text>
                            </Pressable>
                        ))}
                    </View>
                    <View style={{ flexDirection: "row", gap: normalize(20), marginTop: normalize(20) }}>
                        <Button style={s.buttonModalCancel} styleContainer={s.buttonModalCancelContainer} onPress={() => setModalEditar(false)}>
                            <Text style={{ color: gb.gray500, fontWeight: "bold", fontSize: normalize(12) }}>Cancelar</Text>
                        </Button>
                        <Button style={s.buttonModalAccept} styleContainer={s.buttonModalAcceptContainer} onPress={() => abrirCaja()}>
                            <Text style={{ color: gb.gray50, fontWeight: "bold", fontSize: normalize(12) }}>Confirmar</Text>
                        </Button>
                    </View>
                </View>
            </GeneralModal>
