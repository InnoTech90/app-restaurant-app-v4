...
export default class Database {
        ...
        return withDb(async (db) => {
            ...
            return Promise.all(
                renglones.map(async (renglon) => {
                    ...
                    return { ...renglon, articulo: articulo ?? null, complementos: compRows };
                })
            );
        });
    }
    ...
    static async setComandaImpresa(idComanda) {
        return withDb(async (db) => {
            await db.runAsync(
                `UPDATE COMANDA SET ESTATUS = 4 WHERE ID = ?`,
                [idComanda]
            );
        });
    }
    ...
    static async setComandaAbierta(idComanda) {
        return withDb(async (db) => {
            await db.runAsync(
                `UPDATE COMANDA SET ESTATUS = 0 WHERE ID = ?`,
                [idComanda]
            );
        });
    }
    ...
    static async getFormatosPago() {
        return withDb(async (db) => {
            return await db.getAllAsync(`SELECT * FROM METODO_PAGO WHERE ACTIVO = 1 ORDER BY ID ASC`);
        });
    }
    ...
    static async actualizarNota(idComanda, nota) {
        return withDb(async (db) => {
            return await db.runAsync(`UPDATE COMANDA SET NOTA = ? WHERE ID = ?`, [nota, idComanda]);
        });
    }
    ...
    static async finalizarComanda(idComanda, datos) {
        const esPendiente = !datos.formatoPago || datos.formatoPago.toLowerCase() === 'pendiente';
